...
Held-out test split of 213 problems (182 APPS, 23 MBPP, 8 HumanEval+), none of them seen during training. Greedy decoding, 12,288 token budget.

| | stock | distilled |
|---|---:|---:|
| pass@1 | 23.0% | **63.4%** |
| APPS (n=182) | 22.5% | 69.8% |
| hard problems (n=26) | 3.8% | 65.4% |
| responses that terminated | 34.3% | 93.0% |
| mean completion tokens | 9,736 | 3,125 |
| responses with no code block | 138 | 11 |

The gain comes from finishing, not from better algorithms. Stock Nemotron 3 Nano failed 138 of 213 problems by never producing code at all: it reasoned past the token budget and got cut off mid-thought, usually with a correct analysis and no solution. Distillation taught it to stop analyzing and commit to an answer. The effect is largest on hard problems, which is where the base model spirals longest.

Caveats worth stating plainly. This is one training run with one seed and one checkpoint. MBPP sits at 4.3% across every model tested, which is implausible for a model that scores 87.5% on HumanEval+, so something in that part of the harness is probably wrong. Evaluation uses greedy decoding while the corpus was generated at temperature 1.0.
